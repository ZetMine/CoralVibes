...
public class Utils {
    ...
    public final String cvPrefix = ChatColor.WHITE.toString() + ChatColor.BOLD + "CoralVibes >>> ";

    public String kickMessage(String reason) {
        String coralVibes = ChatColor.WHITE.toString() + ChatColor.BOLD + "CoralVibes";
        String kick = ChatColor.WHITE + "Vous avez été expulsé de CoralVibes.";
        String reasonFormat = ChatColor.WHITE.toString() + ChatColor.BOLD + "Raison: " + ChatColor.RED + reason+ChatColor.WHITE;
        String message = """
            ──────────────── %s ────────────────
            ...
            Une erreur ? Contacter sur notre discord!

            ─────────────────────────────────────
            """.formatted(coralVibes, kick, reasonFormat);

        return message;
    }


    public List<Object> getPlayer(String playerName, LuckPerms lpApi) {
        Player player = Bukkit.getPlayer(playerName);

        if (player != null) {
            User userData = lpApi.getUserManager().getUser(player.getUniqueId());
            Group rank = lpApi.getGroupManager().getGroup(userData.getPrimaryGroup());
            String prefix = rank.getCachedData().getMetaData().getPrefix();

            List<Object> playerData = new ArrayList<>();
            playerData.add(playerName);
            playerData.add(prefix);
            playerData.add(rank);

            return playerData;
        } else {
            return null; // Return null if the player is not online
        }
    }

    public boolean isRanked (String playerName, LuckPerms lpApi) {
        Player player = Bukkit.getPlayer(playerName);
        if (player != null) {
            User userData = lpApi.getUserManager().getUser(player.getUniqueId());
            Group rank = lpApi.getGroupManager().getGroup(userData.getPrimaryGroup());
